...
@FallbackDirectory nvarchar(max) = NULL,
...
BEGIN
  ...
  SET @Parameters += ', @FallbackDirectory = ' + ISNULL('''' + REPLACE(@FallbackDirectory,'''','''''') + '''','NULL')
  ...
  BEGIN
    ...
    ELSE
    BEGIN
      INSERT INTO @Directories (ID, DirectoryPath, Mirror, Fallback, Completed)
      SELECT 1, @DefaultDirectory, 0, 0, 0
    END
  END
  ...
  SET @FallbackDirectory = REPLACE(@FallbackDirectory, CHAR(10), '')
  SET @FallbackDirectory = REPLACE(@FallbackDirectory, CHAR(13), '')

  WHILE CHARINDEX(', ', @FallbackDirectory) > 0 SET @FallbackDirectory = REPLACE(@FallbackDirectory,', ',',')
  WHILE CHARINDEX(' ,', @FallbackDirectory) > 0 SET @FallbackDirectory = REPLACE(@FallbackDirectory,' ,',',')

  SET @FallbackDirectory = LTRIM(RTRIM(@FallbackDirectory));

  WITH Directories ( StartPosition, EndPosition, Directory) AS (
    SELECT 1 as StartPosition,
    ISNULL(NULLIF(CHARINDEX(@StringDelimiter, @FallbackDirectory, 1), 0), LEN(@FallbackDirectory) + 1) as EndPosition,
    SUBSTRING(@FallbackDirectory, 1, ISNULL(NULLIF(CHARINDEX(@StringDelimiter, @FallbackDirectory, 1), 0), LEN(@FallbackDirectory) + 1) - 1) as Directory
    WHERE @FallbackDirectory IS NOT NULL
    UNION ALL
    SELECT CAST(EndPosition AS int) + 1 as StartPosition,
           ISNULL(NULLIF(CHARINDEX(@StringDelimiter, @FallbackDirectory, EndPosition + 1), 0), LEN(@FallbackDirectory) + 1) AS EndPosition,
           SUBSTRING(@FallbackDirectory, EndPosition + 1, ISNULL(NULLIF(CHARINDEX(@StringDelimiter, @FallbackDirectory, EndPosition + 1), 0), LEN(@FallbackDirectory) + 1) - EndPosition - 1) as Directory
    FROM Directories
    WHERE EndPosition < LEN(@FallbackDirectory) + 1                      
  )
  INSERT INTO @Directories ( ID, DirectoryPath, Mirror, Fallback, Completed)
  SELECT (SELECT COUNT(*) FROM @Directories) + ROW_NUMBER() OVER(ORDER BY StartPosition asc) as ID,
         Directory,
  ...
  IF EXISTS (SELECT * FROM @Directories WHERE Mirror = 0 AND Fallback = 0 AND (NOT (DirectoryPath LIKE '_:' OR DirectoryPath LIKE '_:\%' OR DirectoryPath LIKE '\\%\%' OR (DirectoryPath LIKE '/%' AND @HostPlatform = 'Linux') OR DirectoryPath = 'NUL') OR DirectoryPath IS NULL OR LEFT(DirectoryPath,1) = ' ' OR RIGHT(DirectoryPath,1) = ' '))
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
  ...
  IF (SELECT COUNT(*) FROM @Directories WHERE Mirror = 0 AND Fallback = 0) <> (SELECT COUNT(*) FROM @Directories WHERE Mirror = 1) AND (SELECT COUNT(*) FROM @Directories WHERE Mirror = 1) > 0
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
  ...
  IF EXISTS (SELECT * FROM @Directories WHERE Mirror = 0 AND Fallback = 0 AND DirectoryPath = 'NUL') AND EXISTS(SELECT * FROM @Directories WHERE Mirror = 0 AND DirectoryPath <> 'NUL')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @Directory is not supported.', 16, 5
  END

  IF EXISTS (SELECT * FROM @Directories WHERE Mirror = 0 AND Fallback = 0 AND DirectoryPath = 'NUL') AND EXISTS(SELECT * FROM @Directories WHERE Mirror = 1)
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
  ...
  IF EXISTS(SELECT * FROM @Directories WHERE Fallback = 1 AND (NOT (DirectoryPath LIKE '_:' OR DirectoryPath LIKE '_:\%' OR DirectoryPath LIKE '\\%\%' OR (DirectoryPath LIKE '/%' AND @HostPlatform = 'Linux')) OR DirectoryPath IS NULL OR LEFT(DirectoryPath,1) = ' ' OR RIGHT(DirectoryPath,1) = ' '))
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @FallbackDirectory is not supported.', 16, 1
  END

  IF EXISTS (SELECT * FROM @Directories WHERE Fallback = 1 GROUP BY DirectoryPath HAVING COUNT(*) <> 1)
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @FallbackDirectory is not supported.', 16, 2
  END

  -- IF @BackupSoftware IN('SQLBACKUP','SQLSAFE') AND (SELECT COUNT(*) FROM @Directories WHERE Fallback = 1) > 1
  -- BEGIN
  --   INSERT INTO @Errors ([Message], Severity, [State])
  --   SELECT 'The value for the parameter @MirrorDirectory is not supported.', 16, 4
  -- END

  IF @FallbackDirectory IS NOT NULL AND SERVERPROPERTY('EngineEdition') = 8
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @FallbackDirectory is not supported.', 16, 5
  END

  IF @FallbackDirectory IS NOT NULL AND @BackupSoftware = 'DATA_DOMAIN_BOOST'
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @FallbackDirectory is not supported.', 16, 6
  END

  -- IF (@BackupSoftware IS NULL AND EXISTS(SELECT * FROM @Directories WHERE Fallback = 1) AND SERVERPROPERTY('EngineEdition') <> 3)
  -- BEGIN
  --   INSERT INTO @Errors ([Message], Severity, [State])
  --   SELECT 'The value for the parameter @FallbackDirectory is not supported. Mirrored backup to disk is only available in Enterprise and Developer Edition.', 16, 8
  -- END
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        -- INSERT INTO @Errors ([Message], Severity, [State])
        -- SELECT 'The directory ' + @CurrentRootDirectoryPath + ' does not exist.', 10, 1

		raiserror('The directory %s does not exist.', 10, 1, @CurrentRootDirectoryPath ) with nowait;
    ...
    IF EXISTS (SELECT * FROM @Directories WHERE Mirror = 0 AND Fallback = 0 AND [Exists] = 0 AND Completed = 1)
    BEGIN

	  raiserror('One or more paths in @Directory does not exist. Trying @FallbackDirectory.', 10, 1) with nowait

      IF EXISTS (SELECT * FROM @Directories WHERE Mirror = 0 AND Fallback = 1 AND [Exists] = 0 AND Completed = 1)
      BEGIN

		raiserror('One or more paths in @FallbackDirectory does not exist.', 10, 1) with nowait;
	
        INSERT INTO @Errors ([Message], Severity, [State])
        SELECT 'One or more paths in both @Directory and @FallbackDirectory do not exist.', 16, 1
      END
	  
      DELETE FROM @Directories
      WHERE Mirror = 0 AND Fallback = 0 AND Completed = 1
    END
    ELSE
    BEGIN
      DELETE FROM @Directories 
      WHERE Mirror = 0 AND Fallback = 1 and Completed = 1
    END

    IF EXISTS (SELECT * FROM @Directories WHERE Mirror = 1 AND [Exists] = 0 AND Completed = 1)
    BEGIN
      INSERT INTO @Errors ([Message], Severity, [State])
      SELECT 'One or more paths in @MirrorDirectory do not exist.', 16, 1
    END
  END
  ...
  IF @URL IS NOT NULL AND @FallbackDirectory IS NOT NULL
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @URL is not supported.', 16, 5
  END
  ----------------------------------------------------------------------------------------------------
  ...
  IF @MirrorURL IS NOT NULL and @FallbackDirectory IS NOT NULL
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @MirrorURL is not supported', 16, 6
  END
  ----------------------------------------------------------------------------------------------------
